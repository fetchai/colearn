    ...
    def wrapper(config):
        ...
        if val is None:
            if config.data == TrainingData.XRAY:
                from examples.xray import split_to_folders
            elif config.data == TrainingData.MNIST:
                from examples.mnist import split_to_folders
            elif config.data == TrainingData.FRAUD:
                from examples.fraud import split_to_folders
            else:
...
def learner_provider(request, data_provider):
    def wrapper(config):
        ...
        if val is None:
            if config.data == TrainingData.XRAY:
                from examples.xray import prepare_single_client, XrayConfig
                model_config = XrayConfig()
            elif config.data == TrainingData.MNIST:
                from examples.mnist import prepare_single_client, MNISTConfig
                model_config = MNISTConfig()
            elif config.data == TrainingData.FRAUD:
                from examples.fraud import prepare_single_client, FraudConfig
